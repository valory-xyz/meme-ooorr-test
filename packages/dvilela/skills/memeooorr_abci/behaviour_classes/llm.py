...
class ActionDecisionBehaviour(
    ...
    def get_event(  # pylint: disable=too-many-locals,too-many-return-statements,too-many-statements
        ...
        summon_token_action_str = ""  # nosec
        if time_since_last_summon >= summon_cooldown_seconds:
            summon_token_action_str = SUMMON_TOKEN_ACTION
            self.context.logger.info(
                f"Summon action is available because time since last summon ({time_since_last_summon}) is greater than or equal to summon cooldown seconds ({summon_cooldown_seconds})"
            )
        else:
            self.context.logger.info(
                f"Summon action is not available because time since last summon ({time_since_last_summon}) is less than summon cooldown seconds ({summon_cooldown_seconds})"
            )
        ...
        try:
            ...
            # decide if we need to pass summon_token_action to the alternative LLM on the basis of summon_cooldown_seconds
            if time_since_last_summon < summon_cooldown_seconds:
                summon_token_action_str = ""  # nosec
                self.context.logger.info(
                    f"Summon action is not available for alternative LLM because time since last summon ({time_since_last_summon}) is less than summon cooldown seconds ({summon_cooldown_seconds})"
                )
            else:
                summon_token_action_str = SUMMON_TOKEN_ACTION
                self.context.logger.info(
                    f"Summon action is available for alternative LLM because time since last summon ({time_since_last_summon}) is greater than or equal to summon cooldown seconds ({summon_cooldown_seconds})"
                )
